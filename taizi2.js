...
const $ = new Env('maotai预约');

const xxx_KEY = 'MAOTAI_KEY';

let MAOTAI_KEY = ($.isNode() ? process.env.HISENSE_CPS : $.getdata("MAOTAI_KEY")) || '';

debug(MAOTAI_KEY)
MAOTAI_KEY = JSON.parse(MAOTAI_KEY)
// 检查项列表
const c = [
    MAOTAI_KEY
];
...
class Encrypt {
    tt() {
        ...
        return F = (M = U).lib, n = F.Base, o = F.WordArray, (M = M.x64 = {}).Word = n.extend({
            ...
            e = e.MD5 = i.extend({
                _doReset: function () {
                    ...
                    var o = this._hash.words, s = t[e + 0], c = t[e + 1], a = t[e + 2], h = t[e + 3], l = t[e + 4],
                        f = t[e + 5], d = t[e + 6], u = t[e + 7], p = t[e + 8], _ = t[e + 9], y = t[e + 10],
                        v = t[e + 11],
            ...
            var t = U, e = t.lib.BlockCipher, r = t.algo, h = [], l = [], f = [], d = [], u = [], p = [], _ = [],
                y = [],
                v = [], g = [];
            !function () {
    ...
    pkcs7padding(text) {
        ...
        const bytesLength = this.tt().enc.Utf8.parse(text).length;
    ...
    aesEncrypt(contentPadding) {
        var key = this.tt().enc.Utf8.parse('qbhajinldepmucsonaaaccgypwuvcjaa');
        var iv = this.tt().enc.Utf8.parse('2018534749963515');
        var contentPadding2 = this.tt().enc.Utf8.parse(this.pkcs7padding(contentPadding));
        const cipher = this.tt().AES.encrypt(contentPadding2, key, {iv: iv, mode: this.tt().mode.CBC});


        return cipher.ciphertext.toString(this.tt().enc.Base64);
    }


    // aesDecrypt(content) {
    //     // AES解密
    //     const decipher = CryptoJS.createDecipheriv('aes-128-cbc', this.key, this.iv);
    //     const text = decipher.update(content, 'base64', 'utf-8') + decipher.final('utf-8');
    //     return text.slice(0, text.length - this.coding.length);
    // }
}
...
function  constructor1(checkItem) {


        this.checkItem = checkItem;
        this.RESERVE_RULE = 0;
        this.mtR = 'clips_OlU6TmFRag5rCXwbNAQ/Tz1SKlN8THcecBp/HGhHdw==';
        this.ITEM_MAP = {
...
function  removeEmptyKeys(obj) {
        return Object.keys(obj).reduce((result, key) => {
            const value = obj[key];
            // 检查值是否为空或假值
            if (value != null && value !== '' && (typeof value !== 'object' || (Array.isArray(value) && value.length > 0) || (typeof value === 'object' && Object.keys(value).length > 0))) {
                result[key] = value;
            }
            return result;
        }, {});
...
function  initHeaders(userId = '1', token = '2', lat  , lng ) {
...
// 获取自1970年1月1日00:00:00 UTC以来的毫秒数
        ...
        const url = `https://static.moutai519.com.cn/mt-backend/xhr/front/mall/index/session/get/${timestampMilliseconds}`;
            ...
            $.get(opt, async (err, resp, data) => {
                try {
                    ...
                    if (data) {
                        ...
                        this.headers["current_session_id"] = data.data.sessionId.toString();
                    }
                } catch (e) {
                    console.log(e)
                }finally {
    ...
    async function getMap(lat  , lng ) {
        ...
        const headers = {
            ...
            'MT-APP-Version': this.mtVersion,
            'MT-Request-ID': `${Date.now()}${Math.floor(Math.random() * (999999999 - 1111111) + 1111111)}${Date.now()}`,
            'Accept-Language': 'zh-CN,zh-Hans;q=1',
            'MT-Device-ID': `${Date.now()}${Math.floor(Math.random() * (999999999 - 1111111) + 1111111)}${Date.now()}`,
        ...
        await  new Promise(resolve => {   $.get(opt,      (err, resp, data) => {

                try {

                    err && $.log(err);

                    uurl = JSON.parse(data).data.mtshops_pc.url
        ...
        let opt2 = {
           ...
           await  new Promise(resolve => { $.get(opt2,   (err, resp, data) => {
                try {
                    err && $.log(err);
...
function maxShop(city, itemCode, pCMap, province, shops) {
        let maxCount = 0;
        let maxShopId = '0';
     ...
     async function distanceShop(itemCode, shops, sourceData, lat , lng ) {
        ...
        for (const shop of shops) {
            ...
            const d = Math.sqrt((parseFloat(lat) - parseFloat(shopInfo.lat)) ** 2 + (parseFloat(lng) - parseFloat(shopInfo.lng)) ** 2);
            tempList.push([d, shopId]);
        }

        tempList.sort((a, b) => a[0] - b[0]);
        return tempList.length > 0 ? tempList[0][1] : '0';
    }
...
// 获取自1970年1月1日00:00:00 UTC以来的毫秒数
        ...
        const url = `https://static.moutai519.com.cn/mt-backend/xhr/front/mall/shop/list/slim/v3/${sessionId}/${province}/${itemCode}/${timestampMilliseconds}`;
       ...
       await new Promise(resolve => {
        ...
        if (reserveRule === 0) {
            ...
            return await distanceShop(itemCode, shops, sourceData, lat, lng);
        }
        if (reserveRule === 1) {
            return maxShop(city, itemCode, pCMap, province, shops);
        }
    }

async function actParams(shopId, itemId) {
        ...
        const s = JSON.stringify(params).replaceAll(":", ": ").replaceAll(",", ", ");

        const act = this.encrypt.aesEncrypt(s).toString('base64');
    ...
    async function reservation(params) {
        ...
        this.headers = removeEmptyKeys(this.headers)
        ...
        let opt = {
            ...
            body: JSON.stringify(restParams).replaceAll(":", ": ").replaceAll(",", ", "),
        }
     await   new Promise(resolve => {
...
$.msg(`🎉 ${$.name}`,'',data  );
    ...
    async  function getUserEnergyAward() {
        // 耐力值
        const cookies = {
    ...
    async function main() {
        ...
        try {
            var pCMap;
            var sourceData;
            await   getMap(lat, lng).then(([pCMap1, sourceData1]) => {
            ...
            initHeaders(userid, token, lng, lat);
            ...
            for (const itemCode of item_codes) {
                ...
                const reservationParams = await actParams(maxShopId, itemCode);
                await reservation(reservationParams)
